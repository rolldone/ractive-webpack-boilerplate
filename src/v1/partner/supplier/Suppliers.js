...
import template from './views/SuppliersView.html';

export const Suppliers = ListTable.extend({
  template,
  isolate : true,
  data : function(){
    return {
  ...
  onconfig : function(){
    let self = this;
    self._super();
    self.root.findComponent('head-menu').setHeader('page_name',gettext("Liste des Fournisseur"));
  },
  getDatas : async function(){
    let self = this;
    try {
      let url = window.HTTP_REQUEST.SUPPLIER_XHR.SUPPLIERS;
  ...
  deleteItem : async function(id){
    let self = this;
    try{
      var url = window.HTTP_REQUEST.SUPPLIER_XHR.DELETE;
      ...
      swalSuccess(gettext("Suppression du fournisseur réalisée"),resData.return);
  ...
  newOn : {
    observeFilterButtonClickListener : function(c,text,object){
      ...
      switch(text){
        case 'ADD':
        return self.dispatch('supplier.new')
        case 'SEARCH':
