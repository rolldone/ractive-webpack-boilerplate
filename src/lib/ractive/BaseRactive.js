...
var BaseRactive = Ractive.extend({
	...
	baseOnConfig : function(){
		/* Create something baseOnConfig */
		let self = this;
		self.initObServer();
	},
	oncomplete : function(){
	...
	baseOnComplete : function(){
		/* Create something baseOnComplete */
	},
	...
	recheckPaginationAfterEmpty : async function(lastPage){
		...
		let newUrlState = self.jsonToQueryUrl(window.location.href,{
			page : self.get('page')
		},null)
